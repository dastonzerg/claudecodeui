...
import { Settings, ArrowUpCircle, Bug, AlertTriangle, ChevronRight } from 'lucide-react';
...
/** Collapsed by default: these are reference links, not daily controls. */
const EXTRAS_STORAGE_KEY = 'sidebar-footer-extras';
...
export default function SidebarFooter({
  ...
  const [showExtras, setShowExtras] = useState(() => {
    try {
      return localStorage.getItem(EXTRAS_STORAGE_KEY) === '1';
    } catch {
      return false;
    }
  });
  ...
  const toggleExtras = () => {
    setShowExtras((current) => {
      const next = !current;
      try {
        localStorage.setItem(EXTRAS_STORAGE_KEY, next ? '1' : '0');
      } catch {
  ...
  // Folding the update notice away must not make a new version invisible, so
  // the collapsed row carries a dot when one is waiting. Deliberately static —
  // a permanently animated element is a real battery/GPU cost for something
  // that sits on screen all day.
  const showUpdateHint = !showExtras && updateAvailable;
  ...
  return (
    <div className="flex-shrink-0" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0)' }}>
      ...
      {/* Collapsible extras: update notice, issue tracker, community. Kept out
          of the way by default; Settings and the version line stay put. */}
      <div className="nav-divider" />

      {/* Desktop extras toggle */}
      <div className="hidden px-2 pt-1.5 md:block">
        ...
        >
          <ChevronRight
            className={`h-3.5 w-3.5 flex-shrink-0 transition-transform ${showExtras ? 'rotate-90' : ''}`}
            aria-hidden
          />
      ...
      <div className="px-3 pt-3 md:hidden">
        ...
        >
          <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-lg bg-background/80">
            <ChevronRight
              className={`h-4 w-4 text-muted-foreground transition-transform ${showExtras ? 'rotate-90' : ''}`}
              aria-hidden
            />
          </div>
          <span className="text-sm font-normal text-foreground">{t('actions.more')}</span>
          {showUpdateHint && (
            <span className="ml-auto h-1.5 w-1.5 flex-shrink-0 rounded-full bg-blue-500" />
          )}
        </button>
            ...
            <div className="hidden px-2 py-1.5 md:block">
              ...
              >
                <div className="relative flex-shrink-0">
                  ...
                  <span className="absolute -right-0.5 -top-0.5 h-1.5 w-1.5 animate-pulse-brief rounded-full bg-blue-500" />
                </div>
