    ...
    for await (const line of rl) {
      ...
      try {
        ...
        if (entry.type === 'response_item' && entry.payload?.type === 'function_call_output') {
          ...
          if (waitExecCallId) {
            const output = extractCodexToolOutput(sanitizeCodexToolOutput(entry.payload.output));
          ...
          messages.push({
            ...
            timestamp: entry.timestamp,
            toolCallId: entry.payload.call_id,
            // sanitize first so image parts become "[image #N]" text rather
            // than being dropped outright by the text-only extraction below.
            output: extractCodexToolOutput(sanitizeCodexToolOutput(entry.payload.output)),
          });
        }
