...
test('provider models are cached for the three-day ttl', async () => {
  ...
  try {
    ...
    // claude, not codex: this fork resolves the Claude catalog through a live
    // SDK control query that spawns a subprocess, so it is served from the TTL
    // cache. Codex reads a local file the CLI keeps current and stays uncached.
...
test('codex provider models are always loaded directly from the provider', async () => {
  ...
  try {
    ...
    // Codex reads ~/.codex/models_cache.json, which the Codex CLI keeps current;
    // caching that behind a multi-day TTL only pins the UI to a stale snapshot.
