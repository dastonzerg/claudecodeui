 ...
 *
 * Claude is deliberately absent: its catalog now comes from a live SDK control
 * query that spawns a subprocess, so it must stay behind the TTL cache.
 */
const UNCACHED_PROVIDERS = new Set<LLMProvider>(['codex']);
