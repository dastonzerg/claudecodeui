...
import {
  getClaudeConfiguredDefaultModel,
  getClaudeSelectedModelFromLines,
...
function readCodexTokenUsage(fileContent: string): TokenUsageResult {
  ...
  for (let index = lines.length - 1; index >= 0; index -= 1) {
    try {
      ...
      // every turn of the session — each one re-sending the whole conversation,
      // cached input included — so it climbs far past the window and pins
      // "% context left" at 0. Fall back to it only for transcripts that
      // predate the per-request figure. Note `cached_input_tokens` is a subset
      // of `input_tokens`, so the two must not be summed.
      const codexUsage = tokenInfo.last_token_usage || tokenInfo.total_token_usage;
      if (codexUsage) {
        inputTokens = readUsageNumber(codexUsage.input_tokens);
        outputTokens = readUsageNumber(codexUsage.output_tokens);
        totalTokens = readUsageNumber(codexUsage.total_tokens)
...
async function readClaudeTokenUsage(
  fileContent: string,
  ...
  for (let index = lines.length - 1; index >= 0; index -= 1) {
    try {
      ...
      sessionModel = (entry.message?.model as string | undefined) ?? null;
  ...
  // Prefer an explicit override, then the real per-model window. An assistant's
  // `message.model` is the bare id (`claude-opus-5`) with any `[1m]` beta
  // stripped, so the window has to come from a model *selection*: the session's
  // own `/model` line when it has one, else Claude Code's saved default (which
  // sessions started on it never record). The saved default is global, so it is
  // only trusted when it names the same model family the session actually ran on.
  ...
  if (Number.isFinite(parsedContextWindow)) {
    ...
    contextWindow = getClaudeContextWindow(selectedModel) ?? getClaudeContextWindow(sessionModel) ?? 160_000;

    // Usage above the resolved window proves the session actually ran on the
    // larger 1M beta, whatever the model strings implied. This is evidence,
    // not inference, so it overrides them.
...
export function createProviderTokenUsageService(
  ...
  return {
    ...
    async getSessionTokenUsage(sessionId: string): Promise<TokenUsageResult> {
      ...
      const fileContent = await dependencies.readTextFile(sessionFilePath);
      return await readClaudeTokenUsage(fileContent, dependencies.getClaudeContextWindow());
    },
  };
}
